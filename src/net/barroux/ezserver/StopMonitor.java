...
class StopMonitor extends Thread {
   ...
   public void run() {
      try {
         ...
         BufferedReader reader = new BufferedReader(new InputStreamReader(accept.getInputStream(), UTF_8));
    ...
    * de fait à communiquer avec un StopMonitor qui s'exécute dans une autre
    * JVM.
    * 
    * @param port
   ...
   static void sendStopCommand(int port, int graceDelay) {
      try {
         ...
         log.info("Could not issue command to server - Probably not running ." + es);
      }
   }
}
